import { checkUser, getUsers, getRoutes, getStops, getUserDuties, assignDuty } from "./api.js";
...
export async function initHome(container) {
  ...
  document.getElementById("user-info").innerText =
    `You are logged in as ${userData.username} (${userData.role})`;
  ...
  if (userData.role === "admin") {
    ...
    formEl.submitCallback = async (newData, valid) => {
      if (!valid) {
        toast.error({ title: "Invalid", description: "Please fix the errors", duration: 10000 });
        return;
      }
      try {
        ...
        toast.success({ title: "Success", description: "Assignments saved", duration: 10000 });
      } catch (err) {
